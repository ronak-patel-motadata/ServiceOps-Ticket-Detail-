import type { RegistryDeployment } from './RegistryDeploymentsListPage';

interface RegistryDeploymentsTableProps {
  ...
  onDeploymentClick?: (deployment: RegistryDeployment) => void;
}

const Dash = () => <span className="text-[12px] text-[#9ca3af]">---</span>;

// Deployment lifecycle status → colored dot (same palette as the other deployment lists).
const statusDot = (s: RegistryDeployment['status']) =>
...
export function RegistryDeploymentsTable({
  deployments,
  selected,
  allSelected,
  onSelectAll,
      ...
      <table className="w-full min-w-[1400px]">
        <thead className="border-b border-[#e5e7eb]">
          <tr className="bg-white">
            ...
            <th className="min-w-[140px] px-4 py-2.5 text-left text-[12px] font-semibold text-[#364658] tracking-wider"><span className="whitespace-nowrap">Total Installations</span></th>
            <th className="min-w-[180px] px-4 py-2.5 text-left text-[12px] font-semibold text-[#364658] tracking-wider"><span className="whitespace-nowrap">Created By</span></th>
            <th className="min-w-[150px] px-4 py-2.5 text-left text-[12px] font-semibold text-[#364658] tracking-wider"><span className="whitespace-nowrap">Configuration Type</span></th>
          </tr>
        </thead>
        <tbody className="divide-y divide-[#e5e7eb] bg-white">
          {deployments.map((d) => (
            <tr key={d.id} className="group hover:bg-[#f9fafb] transition-colors">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-[12px] text-[#364658] tabular-nums">{d.totalInstallations ?? <Dash />}</td>
              <td className="px-4 py-3 whitespace-nowrap text-[12px] text-[#364658]"><span className="block max-w-[170px] truncate" title={d.createdBy}>{d.createdBy}</span></td>
              <td className="px-4 py-3 whitespace-nowrap text-[12px] text-[#364658]">{d.configurationType}</td>
            </tr>
          ))}
        </tbody>
      </table>
